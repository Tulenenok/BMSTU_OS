...
static int __init mod_init(void)
{
    ...
    do
    {
        printk(KERN_INFO " + %s (%d) (%d - state, %d - prio, flags - %x, policy - %d), parent %s (%d), d_name %s",
   ...
   // task = current;
    printk(KERN_INFO "CURRENT + %s (%d) (%d - state, %d - prio, flags - %x, policy - %d), parent %s (%d), d_name %s",
