...
// epoll_fd = epoll_create(0);
	...
	// Add or remove file descriptors to the epoll instance
	...
	for (;;)
	{
		...
		for (int n = 0; n < nfds; ++n)
		{
			if (events[n].data.fd == listen_sock)
			{
				...
				// edge-triggered mode - режим, при котором epoll_wait заблокируется, 
				// даже если в канале остались необработанные данные
				// level-triggered: тригерится каждый раз, когда видит данные в канале   
